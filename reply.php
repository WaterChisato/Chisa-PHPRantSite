...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ⭐⭐⭐ QQ API 验证
    $api = "http://notese.waterchisato.top/proxy.php?qq=" . $qq;
    $data = json_decode(file_get_contents($api), true);

    // API 返回格式：{ code:200, data:{ name, avatar_apiurl_1 } }
    if (!isset($data['data']['name']) || $data['data']['name'] === null || $data['data']['name'] === "") {
        echo "<script>alert('请输入有效的 QQ 号');history.back();</script>";
        exit;
    }

    // 使用 API 返回的昵称和头像
    $nickname = $data['data']['name'];
    $avatar = $data['data']['avatar_apiurl_1'];
...
<div class="container">
    ...
    <form method="post">
        <label>QQ号（自动获取昵称与头像）</label>
        <input id="qq" name="qq" onblur="fetchQQ()">

        <label>昵称（自动填充）</label>
        <input id="nickname" name="nickname" readonly>
